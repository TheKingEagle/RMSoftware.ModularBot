...
namespace ModularBOT.Component.ConsoleScreens
{
    public class UsersScreen : ConsoleScreen
    {
        ...
        public override bool ProcessInput(ConsoleKeyInfo keyinfo)
        {
            ...
            if (keyinfo.Key == ConsoleKey.P || keyinfo.Key == ConsoleKey.LeftArrow)
            {
                ...
                if (ppg != page)
                {
                    ...
                    ClearContents();
                    //RenderContents();
                    countOnPage = PopulateList(d, page, max, ref index, selectionIndex, ref ppg, ref UserList);
                }
            ...
            if (keyinfo.Key == ConsoleKey.Enter)
            {
                ...
                #region SubScreen
                string username = SafeName(UserList, index + selectionIndex);
                if ((d.PermissionManager.DefaultAdmin.EntityID == UserList[index+selectionIndex].Id))
                {
                ...
                int rr = ShowOptionSubScreen($"Editing: {username}", "Please select a new access level", "BlackListed", "Normal", "Command Manager", "Administrator");

                switch (rr)
                {
                    case (1):
                        d.PermissionManager.RegisterEntity(UserList[selectionIndex + index], AccessLevels.Blacklisted);
                        break;
                    case (2):
                        if (d.PermissionManager.IsEntityRegistered(UserList[selectionIndex + index]))
                        {
                            d.PermissionManager.DeleteEntity(UserList[selectionIndex + index]);
                        }
                        break;
                    case (3):
                        d.PermissionManager.RegisterEntity(UserList[selectionIndex + index], AccessLevels.CommandManager);
                        break;
                    case (4):
                        d.PermissionManager.RegisterEntity(UserList[selectionIndex + index], AccessLevels.Administrator);
                        break;
                    default:
                        break;
                }
        ...
        private int PopulateList(DiscordNET discord, short page, short max, ref int index, int selectionIndex, ref int ppg, ref List<SocketGuildUser> users)
        {
            int countOnPage;
        ...
        private void WriteListEntry(int selectionIndex, int countOnPage, List<SocketGuildUser> users, int i)
        {
            ...
            string accesslevel = d.PermissionManager.GetAccessLevel(users.ElementAt(i)).ToString().PadRight(16,'\u2005');
        ...
        private string SafeName(List<SocketGuildUser> users, int i)
        {
