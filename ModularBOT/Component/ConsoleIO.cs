...
namespace ModularBOT.Component
{
    public class ConsoleIO
    {
        ...
        public static string WordWrap(string paragraph, int consoleoffset = 23)
        {
        ...
        /// </summary>
        public void GetConsoleInput(ref bool ShutdownCalled, ref bool RestartRequested)
        {
            ulong chID = 0;
            ...
            while (true)
            {
                ...
                Console.SetCursorPosition(cl, ct);//reset
                ...
                if (input.ToLower() == "stopbot")
                {
                    ...
                    WriteEntry(new LogMessage(LogSeverity.Warning, "Session", "Ending session. and closing program in 3..."), ConsoleColor.Black, false);
                    Console.SetCursorPosition(cl, ct);//reset
                    //_client.SetGameAsync("");
                    Thread.Sleep(1000);

                    WriteEntry(new LogMessage(LogSeverity.Warning, "Session", "Ending session. and closing program in 2..."), ConsoleColor.Black, false);
                    Console.SetCursorPosition(cl, ct);//reset
                    //_client.SetStatusAsync(UserStatus.Invisible);
                    Thread.Sleep(1000);
                    WriteEntry(new LogMessage(LogSeverity.Warning, "Session", "Ending session. and closing program in 1..."), ConsoleColor.Black, false);
                    Console.SetCursorPosition(cl, ct);//reset
                    //_client.StopAsync();
                    Thread.Sleep(1000);
